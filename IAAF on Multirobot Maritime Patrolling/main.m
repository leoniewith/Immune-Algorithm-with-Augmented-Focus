...
while iter <= MaxGeneration
    ...
    DecodePop = DecodeFunction(DecodePop,XVessel,YVessel,RVessel,GenotypeLength,size(DecodePop,1),VarietyNumber,MaxRobotNo,DistanceWeight,PortPosition);
    sub_array = DecodePop(:,1:VarietyNumber*sum(GenotypeLength));  %提取sub_array的行并将其重新排列为12x10的矩阵
    
    % Augmented Focus
    % Dominance Inverse Frequency-Inspired Mutation
    C = DecodePop(:,end-2:end);
    for i = 1:PopulationSize
        A = DecodePop(i,end-2:end);
        B = DecodePop(setdiff(1:PopulationSize,i),end-2:end);
        count = 0;
        for io = 1:size(B,1)
            index= A<B(io,:);
            sumindex = sum(index');
            exist = find(sumindex == 3);
            if ~isempty(exist)
                count=count+1;
            end
        end
        solve = count/size(B,1);
        occurence_all = zeros(1,VarietyNumber*sum(GenotypeLength));
        probability_all = zeros(1,VarietyNumber*sum(GenotypeLength));
        for column = 1:VarietyNumber*sum(GenotypeLength)
            if DecodePop(i,column) == 0
                occurence = PopulationSize-sum(DecodePop(:,column));
            else
                occurence = sum(DecodePop(:,column));
            end
            occurence_all(1,column) = occurence;
            proall = [proall solve / (occurence_all(1,column)/PopulationSize)];
            probability = 0.5 - solve / (occurence_all(1,column)/PopulationSize);
            probability_all(1,column) = probability;
        end
        % Self-Attention-Based Mutation
        if iter == 1
            W1 = rand(VarietyNumber, VarietyNumber);
            W2 = rand(VarietyNumber, VarietyNumber);
            W3 = rand(VarietyNumber, VarietyNumber);
        else
            if i <= 5
                W1 = W4_array{iter-1, i };
                W2 = W5_array{iter-1, i };
            else
                W1 = rand(VarietyNumber, VarietyNumber);
                W2 = rand(VarietyNumber, VarietyNumber);
                W3 = rand(VarietyNumber, VarietyNumber);
        ...
        W3_array{i,iter} = W3;
        row = sub_array(i,:);
        b = reshape(row, sum(GenotypeLength), VarietyNumber);
        Q = b * W1;
        K = b * W2;
        V = b * W3;
        t = size(Q);
        ...
        K = transpose(K);
        a = (Q * K) / sqrt(first);
        softmax_a = zeros(size(a));
        for j = 1:first
            softmax_a(j,:) = exp(a(j,:)) ./ sum(exp(a(j,:)));
        end
        for j = 1:sum(GenotypeLength)
            softmax_a(sum(GenotypeLength)+1, j) = sum(softmax_a(1:sum(GenotypeLength),j)) / sum(GenotypeLength);
        end
        for imu = 1:VarietyNumber*sum(GenotypeLength)
            if rand <probability_all(1, imu)
                if rand < softmax_a(sum(GenotypeLength)+1, floor((imu-1)/sum(GenotypeLength)) + 1)
                     DecodePop(i,imu) = 1 - DecodePop(i,imu);
                end
            end
        end
    end
    ...
    xuhao = (1:size(DecodePop,1))';
    DecodePop_index = [xuhao,DecodePop];
    DecodePop_index = sortrows(DecodePop_index,size(DecodePop,2)-1);
    for i = 1:5
        W4_array{iter, i} = W1_array{DecodePop_index(i), iter};
    end
    DecodePop_index = sortrows(DecodePop_index,size(DecodePop,2));
    for i = 1:5
        W5_array{iter, i} = W2_array{DecodePop_index(i), iter};
    end
    % Evaluation
    pa = DecodePop(:,(sum(GenotypeLength)+length(GenotypeLength))*length(XVessel)+1:(sum(GenotypeLength)+length(GenotypeLength))*length(XVessel)+3);
